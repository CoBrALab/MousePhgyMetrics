...
# ARG_OPTIONAL_SINGLE([peak_detection_parameter_csv],[],[Parameters that determine the which peaks in the trace are counted as breaths/heart beats. Refer to the scipy.signal.find_peaks() documentation for the full list of parameters. See README for the default values for each data type.],[None])
...
_arg_peak_detection_parameter_csv="None"
...
if [ $_arg_data_type = "respiration" ]; then
   echo "Running respiration analysis..."
   python3.8 $scriptdir/respiration_functions.py $_arg_analysis_type $_arg_input_trace $_arg_tot_length_seconds $_arg_output_path $_arg_output_image_type \
			$_arg_peak_detection_parameter_csv $_arg_invert_trace_boolean $_arg_window_length $_arg_fmri_censoring_mask_csv $_arg_fmri_tr $_arg_average_metrics_window_length $_arg_average_metrics_window_overlap
   echo "Analysis of respiration data complete"
elif [ $_arg_data_type = "plethysmography" ]; then
   echo "Running plethysmography analysis..."
    python3.8 $scriptdir/cardiac_functions.py $_arg_analysis_type $_arg_input_trace $_arg_tot_length_seconds $_arg_output_path $_arg_output_image_type \
			$_arg_peak_detection_parameter_csv $_arg_invert_trace_boolean $_arg_window_length $_arg_fmri_censoring_mask_csv $_arg_fmri_tr $_arg_average_metrics_window_length $_arg_average_metrics_window_overlap
  echo "Analysis of plethysmography data complete"
else 
	echo "The provided data_type is not valid. Specify either 'respiration' or 'plethysmography' only. "
fi
